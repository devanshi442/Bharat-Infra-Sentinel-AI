      ...
      <header className="border-b border-slate-200 bg-white sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/" className="text-brand-primary/55 hover:text-brand-deep transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <h1 className="font-display font-semibold text-lg text-brand-deep">{t('title')}</h1>
          </div>
          <LanguageSwitcher />
        </div>
      </header>
          ...
          <div className="space-y-6">
            ...
            <section className="bg-white rounded-2xl border border-slate-200 p-6">
              ...
              {preview ? (
                ...
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="w-full h-48 rounded-xl border-2 border-dashed border-slate-200 hover:border-brand-accent hover:bg-brand-light/35 transition-colors flex flex-col items-center justify-center gap-2 text-slate-400"
                >
            ...
            <section className="bg-white rounded-2xl border border-slate-200 p-6">
              ...
              {location ? (
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-sm text-brand-medium font-medium bg-brand-light border border-brand-medium/30 rounded-lg px-3 py-2.5 flex-1">
                    <MapPin className="w-4 h-4 shrink-0" />
                    {t('location_captured', 'Location captured')} ({location.lat.toFixed(4)}, {location.lng.toFixed(4)})
                  </div>
                  <button
                    onClick={() => setLocation(null)}
                    className="text-xs text-slate-500 hover:text-slate-800 border border-slate-200 rounded-lg px-3 py-2.5 transition-colors whitespace-nowrap"
                  >
                    {t('reset', 'Reset')}
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 border border-slate-200 hover:border-brand-accent text-slate-700 font-medium px-4 py-2.5 rounded-lg transition-colors disabled:opacity-50"
                >
                  {status === 'locating'
              ...
              <input
                ...
                className="mt-3 w-full text-sm border border-slate-200 rounded-lg px-3 py-2.5 focus:border-brand-accent outline-none"
              />
            </section>
            ...
            <section className="bg-white rounded-2xl border border-slate-200 p-6">
              ...
              <textarea
                ...
                rows={3}
                className={`w-full text-sm border rounded-lg px-3 py-2.5 outline-none resize-none transition-colors ${isRecording ? 'border-red-300 bg-red-50/30' : 'border-slate-200 focus:border-brand-accent'
                  }`}
              />
            </section>
            ...
            <button
              ...
              disabled={!canSubmit}
              className="w-full flex items-center justify-center gap-2 bg-brand-accent hover:bg-brand-accent/90 disabled:bg-slate-100 disabled:cursor-not-allowed disabled:text-slate-400 text-white font-medium px-6 py-4 rounded-xl transition-colors"
            >
              {status === 'submitting' ? (
  ...
  return (
    <div className="space-y-5">
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className={`border-b px-6 py-4 flex items-center gap-2.5 ${result.offline ? 'bg-amber-50 border-amber-200' : 'bg-brand-light border-brand-medium/20'}`}>
          <CheckCircle2 className={`w-5 h-5 ${result.offline ? 'text-amber-600' : 'text-brand-medium'}`} />
          <span className={`font-medium ${result.offline ? 'text-amber-800' : 'text-brand-deep'}`}>
          ...
          {!result.offline && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-brand-light/30 rounded-xl p-4">
                  <div className="text-xs text-slate-500 mb-1">{t('failure_risk_label')}</div>
                  <div className="font-mono font-semibold text-2xl text-brand-deep">{result.failure_probability_30d}%</div>
                </div>
                <div className="bg-brand-light/30 rounded-xl p-4">
                  <div className="text-xs text-slate-500 mb-1">{t('routed_to')}</div>
                  <div className="font-medium text-sm text-brand-deep leading-snug pt-1.5">{result.assigned_department_translated || result.assigned_department}</div>
                </div>
              </div>
      ...
      <div className="flex gap-3">
        <button
          onClick={onReset}
          className="flex-1 bg-white hover:bg-slate-100 border border-slate-200 text-brand-primary font-medium px-6 py-3.5 rounded-xl transition-colors"
        >
          {t('report_another_issue', 'Report Another Issue')}
        </button>
        <Link
          to="/dashboard"
          className="flex-1 text-center border border-slate-200 hover:bg-brand-light text-brand-primary font-medium px-6 py-3.5 rounded-xl flex items-center justify-center transition-colors"
        >
          {t('view_dashboard_btn', 'View Dashboard')}
        </Link>
      </div>
    </div>
  )
